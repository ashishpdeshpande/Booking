...
const AllRooms = () => {
  ...
  const { rooms, navigate, currency } = useAppContxt();
  ...
  const handleSortChange = (sortOptins) => {
    setSelectedSort(sortOptins);
  };

  // Check if room matches the selected room type
  ...
  // Check if room matches the selected price ranges
  ...
  // Sort rooms based on selected sort options
  const sortRooms = (a, b) => {
          ...
          <div className="px-5 pt-5 pb-7">
            ...
            {sortOptins.map((options, index) => (
              <RadioButton
                ...
                onChange={() => handleSortChange(options)}
              />
            ))}
          </div>
        </div>
